...
$("h2").css({"color" : "purple" ,"margin-top" : "0px"});
...
$('#list-3 > li:first').css({"color" : "brown"});
  // for all li with index > 5 set color to #ccc
$('#list-3 > *:gt(5)').css({"color" : "#ccc"});
...
console.log($("[name='checked_checkbox']"));
// show all images with a cat
$("[alt*='cat']").css({"display" : "block"});
//$("[src*='cat']").css({"display" : "block"});
// ******************************************************
// is(), not(), has()
// eq(), first(), last()
...
$("[src*='img']").closest('div').first().css({"float" : "left", "border"  : "solid  5px red"});
